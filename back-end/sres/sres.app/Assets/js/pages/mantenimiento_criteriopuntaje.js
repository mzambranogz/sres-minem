...
var consultar = () => {
    ...
    let url = `${baseUrl}api/puntaje/buscarcriteriopuntaje?${queryParams}`;
...
var renderizar = (data, cantidadCeldas, pagina, registros) => {
    ...
    if (deboRenderizar) {
        contenido = data.map((x, i) => {
            ...
            let colCodigo = `<td class="text-center" data-encabezado="Código" scope="row"><span>${(`${formatoCodigo}${x.ID_CRITERIO}${x.ID_DETALLE}`).split('').reverse().join('').substring(0, formatoCodigo.length).split('').reverse().join('')}</span></td>`;
            let colNombres = `<td class="text-left" data-encabezado="Descripción">${x.DESCRIPCION}</td>`;
            let colPuntaje = `<td class="text-center" data-encabezado="Puntaje">${x.PUNTAJE}</td>`;
            let colCriterio = `<td class="text-left" data-encabezado="Criterio">${x.CRITERIO}</td>`;
            let btnCambiarEstado = `${[0, 1].includes(x.FLAG_ESTADO) ? "" : `<a class="dropdown-item estilo-01 btnCambiarEstado" href="#" data-id="${x.ID_CRITERIO}-${x.ID_DETALLE}" data-estado="${x.FLAG_ESTADO}"><i class="fas fa-edit mr-1"></i>Eliminar</a>`}`;
            let btnEditar = `<a class="dropdown-item estilo-01 btnEditar" href="#" data-id="${x.ID_CRITERIO}-${x.ID_DETALLE}" data-toggle="modal" data-target="#modal-mantenimiento"><i class="fas fa-edit mr-1"></i>Editar</a>`;
            ...
            let fila = `<tr>${colNro}${colCodigo}${colNombres}${colPuntaje}${colCriterio}${colOpciones}</tr>`;
            return fila;
        }).join('');
    };
...
var eliminar = () => {
    ...
    let data = { ID_CRITERIO: idEliminar.split('-')[0], ID_DETALLE: idEliminar.split('-')[1], USUARIO_GUARDAR: idUsuarioLogin };
    let init = { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) };
    let url = `${baseUrl}api/puntaje/cambiarestadocriteriopuntaje`;
    fetch(url, init)
        .then(r => r.json())
        .then(j => {
...
var consultarDatos = (element) => {
    ...
    $('#exampleModalLabel').html('ACTUALIZAR PUNTAJE');
    //$('#cbo-criterio').parent().parent().hide();
    $('#cbo-criterio').prop('disabled', true);

    let id = $(element).attr('data-id');
    let url = `${baseUrl}api/puntaje/obtenercriteriopuntaje?idcriterio=${id.split('-')[0]}&iddetalle=${id.split('-')[1]}`;
    fetch(url)
    .then(r => r.json())
    .then(j => {
        cargarDatos(j);
...
var cargarDatos = (data) => {
    $('#frm').data('id', data.ID_DETALLE);
    $('#txt-nombre').val(data.DESCRIPCION);
    $('#txt-puntaje').val(data.PUNTAJE);
...
var guardar = () => {
    $('.alert-add').html('');
    let arr = [];
    if ($('#txt-nombre').val().trim() === "") arr.push("Ingrese la descripción del puntaje");
    if ($('#txt-puntaje').val().trim() === "") arr.push("Ingrese el puntaje");
    ...
    let id = $('#frm').data('id');
    let descripcion = $('#txt-nombre').val();
    let puntaje = $('#txt-puntaje').val();
    let criterio = $(`#cbo-criterio`).val();

    let url = `${baseUrl}api/puntaje/guardarcriteriopuntaje`;
    let data = { ID_CRITERIO: criterio, ID_DETALLE: id == null ? -1 : id, DESCRIPCION: descripcion, PUNTAJE: puntaje, USUARIO_GUARDAR: idUsuarioLogin };
    let init = { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) };
...
var nuevo = () => {
    ...
    $('#exampleModalLabel').html('REGISTRAR PUNTAJE');
}
