...
var renderizar = (data, cantidadCeldas) => {
    ...
    if (deboRenderizar) {
        contenido = data.DATA.map((x, i) => {
            ...
            //let colSello = `<td class="text-center" data-encabezado="Reconocimiento"><img class="img-fluid medal-sres" src="${baseUrl}${$('#ruta_rec').val().replace('{0}', x.ID_PREMIACION)}/${x.ARCHIVO_BASE}" alt="" data-toggle="tooltip" data-placement="top" title="Reconocimiento oro ${x.ESTRELLA == null ? 'sin estrellas' : x.ESTRELLA == 0 ? 'sin estrellas' : `con ${x.ESTRELLA}`}">&nbsp;${x.FLAG_MEJORACONTINUA == '1' || x.FLAG_EMISIONESMAX == '1' || x.PREMIO_MEDMIT > 0 ? `<img class="img-fluid medal-sres" src="${baseUrl}Assets/images/gei.png" alt="" data-toggle="tooltip" data-placement="top" title="Premiación especial">` : ``}</td>`;
            let colSello = `<td class="text-center" data-encabezado="Reconocimiento"><img class="img-fluid medal-sres" src="${baseUrl}${$('#ruta_rec').val().replace('{0}', x.ID_INSIGNIA)}/${x.INSIGNIA.ARCHIVO_BASE}" alt="" data-toggle="tooltip" data-placement="top" title="${x.ID_INSIGNIA == 1 ? 'Sin categoría' : `Reconocimiento categoría ${x.INSIGNIA.NOMBRE.toLowerCase()}`}">&nbsp;${x.FLAG_MEJORACONTINUA == '1' || x.FLAG_EMISIONESMAX == '1' || x.PREMIO_MEDMIT > 0 ? `<img class="img-fluid medal-sres" src="${baseUrl}Assets/images/gei.png" alt="" data-toggle="tooltip" data-placement="top" title="Premiación especial">` : ``}</td>`;
            ...
            let colPuntaje = `<td data-encabezado="Puntaje"><div class="text-right">${x.PUNTAJE}</div></td>`;
