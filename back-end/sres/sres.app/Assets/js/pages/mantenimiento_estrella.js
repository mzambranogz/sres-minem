...
var consultar = () => {
    ...
    let url = `${baseUrl}api/estrella/buscarestrella?${queryParams}`;
...
var renderizar = (data, cantidadCeldas, pagina, registros) => {
    ...
    if (deboRenderizar) {
        contenido = data.map((x, i) => {
            ...
            let colCodigo = `<td class="text-center" data-encabezado="Código" scope="row"><span>${(`${formatoCodigo}${x.ID_ESTRELLA}`).split('').reverse().join('').substring(0, formatoCodigo.length).split('').reverse().join('')}</span></td>`;
            let colNombres = `<td class="text-left" data-encabezado="Nombre">${x.NOMBRE}</td>`;
            let btnCambiarEstado = `${[0, 1].includes(x.FLAG_ESTADO) ? "" : `<a class="dropdown-item estilo-01 btnCambiarEstado" href="#" data-id="${x.ID_ESTRELLA}" data-estado="${x.FLAG_ESTADO}"><i class="fas fa-edit mr-1"></i>Eliminar</a>`}`;
            let btnEditar = `<a class="dropdown-item estilo-01 btnEditar" href="#" data-id="${x.ID_ESTRELLA}" data-toggle="modal" data-target="#modal-mantenimiento"><i class="fas fa-edit mr-1"></i>Editar</a>`;
...
var eliminar = () => {
    ...
    let data = { ID_ESTRELLA: idEliminar, USUARIO_GUARDAR: idUsuarioLogin };
    let init = { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) };
    let url = `${baseUrl}api/estrella/cambiarestadoestrella`;
    fetch(url, init)
        .then(r => r.json())
        .then(j => {
...
var nuevo = () => {
    ...
    $('#exampleModalLabel').html('REGISTRAR ESTRELLA');
}
...
var consultarCriterio = (element) => {
    ...
    $('#btnGuardar').show();
    $('#btnGuardar').next().html('Cancelar');
    $('#exampleModalLabel').html('ACTUALIZAR ESTRELLA');

    let id = $(element).attr('data-id');
    let url = `${baseUrl}api/estrella/obtenerestrella?id=${id}`;
...
var guardar = () => {
    $('.alert-add').html('');
    let arr = [];
    if ($('#txt-nombre').val().trim() === "") arr.push("Ingrese el nombre de la estrella");
    ...
    let url = `${baseUrl}api/estrella/guardarestrella`;
    let data = { ID_ESTRELLA: id == null ? -1 : id, NOMBRE: nombre, USUARIO_GUARDAR: idUsuarioLogin };
    let init = { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) };
    fetch(url, init)
    .then(r => r.json())
    .then(j => {
