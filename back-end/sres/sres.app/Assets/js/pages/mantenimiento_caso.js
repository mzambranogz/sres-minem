...
var renderizar = (data, cantidadCeldas, pagina, registros) => {
    ...
    if (deboRenderizar) {
        contenido = data.map((x, i) => {
            ...
            let colCodigo = `<td class="text-center" data-encabezado="Código" scope="row"><span>${(`${formatoCodigo}${x.ID_CRITERIO}${x.ID_CASO}`).split('').reverse().join('').substring(0, formatoCodigo.length).split('').reverse().join('')}</span></td>`;
            let colNombres = `<td class="text-left" data-encabezado="Nombre">${x.NOMBRE}</td>`;
            let colCriterio = `<td class="text-left" data-encabezado="Criterio">${x.NOMBRE_CRI}</td>`;
            ...
            let btnEditar = `<a class="dropdown-item estilo-01 btnEditar" href="#" data-id="${x.ID_CRITERIO}-${x.ID_CASO}" data-toggle="modal" data-target="#modal-mantenimiento"><i class="fas fa-edit mr-1"></i>Editar</a>`;
...
var eliminar = () => {
    ...
    let data = { ID_CRITERIO: idEliminar.split('-')[0], ID_CASO: idEliminar.split('-')[1], USUARIO_GUARDAR: idUsuarioLogin };
    let init = { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) };
    let url = `${baseUrl}api/caso/cambiarestadocaso`;
    fetch(url, init)
        .then(r => r.json())
        .then(j => {
...
var nuevo = () => {
    ...
    $('#exampleModalLabel').html('REGISTRAR CASO');
}
