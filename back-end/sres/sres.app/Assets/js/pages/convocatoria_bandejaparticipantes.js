...
var renderizar = (data, cantidadCeldas) => {
        ...
        if (deboRenderizar) {
            ...
            let colAspiraciones = `<td class="text-center" data-encabezado="Imagen"><img src="${baseUrl}${$('#ruta').val().replace('{0}', x.ASPIRACION)}/${x.ARCHIVO_BASE == null ? '' : x.ARCHIVO_BASE}" width="50%" height="auto"></td>`;
            ...
            let OpcioneEta6 = `<div class="dropdown-menu">${btnSeguimiento}</div>`;
            let OpcioneEta7 = `<div class="dropdown-menu">${btnSeguimiento}</div>`;
            let OpcioneEta8 = `<div class="dropdown-menu">${idRolLogin == 2 ? btnEvaluarCriterios : ''}${btnSeguimiento}</div>`;
