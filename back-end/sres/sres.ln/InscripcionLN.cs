        ...
        public bool AnularInscripcion(InscripcionBE inscripcion)
        {
            ...
            try
            {
                ...
                using (OracleTransaction ot = cn.BeginTransaction())
                {
                    ...
                    if (seGuardo) seGuardo = convocatoriaDA.GuardarResultadoReconocimiento(new ReconocimientoBE { ID_INSCRIPCION = inscripcion.ID_INSCRIPCION, ID_INSIGNIA = 1, PUNTAJE = 0, ID_ESTRELLA = 1, EMISIONES = 0, ENERGIA = 0, COMBUSTIBLE = 0, FLAG_MEJORACONTINUA = "0", USUARIO_GUARDAR = inscripcion.USUARIO_GUARDAR }, cn);
                    if (seGuardo)
                    {
